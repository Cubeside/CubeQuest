...
public class DailyQuestData {
    ...
    public DailyQuestData(int id, List<Integer> questIds, String dateString, Date nextDayDate) {
        ...
        for (int i = 0; i < this.quests.length; i++) {
            this.quests[i] = questIds.get(i) == null ? null
                    : QuestManager.getInstance().getQuest(questIds.get(i));
        }
    ...
    public String toString() {
        return "DailyQuestData[id:" + this.id + ", dateString:\"" + this.dateString
                + "\" nextDayDate:" + this.nextDayDate + " quests:"
                + Arrays.stream(this.quests).map(q -> q == null ? "null" : ("" + q.getId()))
