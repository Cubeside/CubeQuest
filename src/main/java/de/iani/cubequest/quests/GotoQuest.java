...
public class GotoQuest extends ServerDependendQuest {
    ...
    public boolean onPlayerMoveEvent(PlayerMoveEvent event, QuestState state) {
        ...
        if (Math.abs(event.getTo().getX() - x) > tolarance
                || Math.abs(event.getTo().getY() - y) > tolarance
    ...
    public boolean isLegal() {
        return world != null && tolarance >= 0
                && (!isForThisServer() || Bukkit.getWorld(world) != null);
    }
    ...
    public List<BaseComponent[]> getQuestInfo() {
        ...
        result.add(new ComponentBuilder(ChatColor.DARK_AQUA + "Zu erreichender Ort: "
                + ChatAndTextUtil.getLocationInfo(world, x, y, z)).create());
    ...
    public Location getTargetLocation() {
        return isForThisServer() && world != null ? new Location(Bukkit.getWorld(world), x, y, z)
                : null;
    }
