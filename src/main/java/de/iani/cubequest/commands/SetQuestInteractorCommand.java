...
public class SetQuestInteractorCommand extends SubCommand implements Listener {
    ...
    public void onPlayerInteractInteractorEvent(PlayerInteractInteractorEvent event) {
        if (currentlySelectingInteractor.remove(event.getPlayer().getUniqueId())) {
            Bukkit.dispatchCommand(event.getPlayer(), "quest setInteractor "
                    + InteractorType.fromClass(event.getInteractor().getClass()) + event.getInteractor().getIdentifier().toString());
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String commandString,
        ...
        try {
            ...
            ChatAndTextUtil.sendWarningMessage(sender, "\"" + identifierString + "\" ist kein gültiger Identifier für einen Interactor vom Typ " + type + ": " + e.getMessage());
            return true;
        }

        Interactor interactor = CubeQuest.getInstance().getInteractorCreator().createInteractor(type, identifier);
        if (interactor == null) {
