...
public class AddOrRemoveEntityTypeCombinationForSpecificationCommand extends SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String commandString,
        ...
        if (!args.hasNext()) {
            ChatAndTextUtil.sendWarningMessage(sender, "Bitte gib die Entity-Typen an, die " + (add? "hinzugefügt" : "entfernt") + " werden sollen.");
            return true;
        }
        ...
        switch (requiredFor) {
            case KILL_ENTITIES:
                ...
                result = add? killEntitiesInstance.adEntityTypeCombination(ec) : killEntitiesInstance.removeEntityTypeCombination(ec);
                break;
            default:
                assert(false);
                return false;
