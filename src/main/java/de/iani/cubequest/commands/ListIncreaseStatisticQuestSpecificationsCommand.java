...
public class ListIncreaseStatisticQuestSpecificationsCommand extends SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String commandString,
        ...
        List<Component> list = IncreaseStatisticQuestPossibilitiesSpecification.getInstance().getSpecificationInfo();
        if (list.isEmpty()) {
            ChatAndTextUtil.sendNormalMessage(sender, "Es gibt keine Statistik-Quest-Möglichkeiten.");
            return true;
        }

        ChatAndTextUtil.sendMessagesPaged(sender, list, args.getNext(1) - 1, "Statistik-Quest-Möglichkeiten",
                FULL_COMMAND);
