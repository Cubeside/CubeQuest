...
public class SetFailAfterSemiSuccessCommand extends AssistedSubCommand {
    ...
    static {
        parameterDefiners = new ParameterDefiner[] {
                ...
                new ParameterDefiner(ParameterType.BOOLEAN, "FailAfterSemiSuccess", parsed -> null)};
        
        propertySetter = parsed -> {
            ((ComplexQuest) parsed[1]).setFailAfterSemiSuccess((Boolean) parsed[2]);
            return null;
        };
        
        successMessageProvider = parsed -> "FailAfterSemiSuccess für Quest " + ((ComplexQuest) parsed[1]).getId() + " auf " + parsed[2] + " gesetzt.";
    }
    
    public SetFailAfterSemiSuccessCommand() {
        super("quest setFailAfterSemiSuccess", ACCEPTING_SENDER_CONSTRAINT, parameterDefiners, propertySetter, successMessageProvider);
    }
