...
public class SetFailAfterSemiSuccessCommand extends AssistedSubCommand {
    ...
    static {
        parameterDefiners = new ParameterDefiner[] {
                new ParameterDefiner(ParameterType.CURRENTLY_EDITED_QUEST, "Quest",
                        parsed -> (!(parsed[1] instanceof ComplexQuest)
                                ...
                                : null)),
                new ParameterDefiner(ParameterType.BOOLEAN, "FailAfterSemiSuccess",
                        parsed -> null)};
        ...
        successMessageProvider = parsed -> "FailAfterSemiSuccess für Quest "
                + ((ComplexQuest) parsed[1]).getId() + " auf " + parsed[2] + " gesetzt.";
    }
    
    public SetFailAfterSemiSuccessCommand() {
        super("quest setFailAfterSemiSuccess", ACCEPTING_SENDER_CONSTRAINT, parameterDefiners,
