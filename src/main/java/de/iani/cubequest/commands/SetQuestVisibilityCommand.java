...
public class SetQuestVisibilityCommand extends SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String commandString,
        ...
        if (!args.hasNext()) {
            ChatAndTextUtil.sendWarningMessage(sender, "Bitte gib an, ob die Quest für Spieler sichtbar sein soll (true | false). (Derzeit: " + visible + ")");
            return true;
        }
        ...
        if (Arrays.asList(new String[] {"t", "true", "y", "yes", "j", "ja"}).contains(arg.toLowerCase())) {
            ...
            ChatAndTextUtil.sendWarningMessage(sender, "Bitte gib an, ob die Quest auf sichtbar gesetzt werden soll (true | false).");
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, ArgsParser args) {
        String arg = args.getNext("");
        List<String> result = new ArrayList<>(Arrays.asList(new String[] {"true", "false", "yes", "no", "ja", "nein"}));
        result.removeIf(s -> {
