...
public class SetRewardIntCommand extends SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias,
        ...
        if (newValue < 0) {
            ChatAndTextUtil.sendWarningMessage(sender, "Bitte gib die Anzahl an " + attribute.name
                    + " als nicht-negative Ganzzahl an.");
            return true;
        }
        
        Reward formerReward = success ? quest.getSuccessReward() : quest.getFailReward();
        int newCubes = attribute == Attribute.CUBES ? newValue
                : formerReward == null ? 0 : formerReward.getCubes();
        int newQuestPoints = attribute == Attribute.QUEST_POINTS ? newValue
                : formerReward == null ? 0 : formerReward.getQuestPoints();
        int newXp = attribute == Attribute.XP ? newValue
        ...
        if (resultReward.isEmpty()) {
            ChatAndTextUtil.sendNormalMessage(sender, (success ? "Erfolgsbelohnung" : "Trostpreis")
                    + " für " + quest.getTypeName() + " [" + quest.getId() + "] entfernt.");
        } else {
            ChatAndTextUtil.sendNormalMessage(sender,
                    attribute.name + " in " + (success ? "Erfolgsbelohnung" : "Trostpreis")
