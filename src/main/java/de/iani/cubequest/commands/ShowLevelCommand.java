...
public class ShowLevelCommand extends SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String commandString,
        ...
        ChatAndTextUtil.sendNormalMessage(sender, (sender == player ? "Du hast " : (player.getName() + " hat ")) + xp
                + " Quest-XP und " + (sender == player ? "bist" : "ist") + " damit Level " + level + ".");

        if (player == sender || sender.hasPermission(CubeQuest.SEE_PLAYER_INFO_PERMISSION)) {
            ChatAndTextUtil.sendNormalMessage(sender,
                    (player == sender ? "Dir" : player.getName()) + " fehl" + (requiredXp - xp == 1 ? "t" : "en")
                            + " noch " + (requiredXp - xp) + " Quest-XP zum nächsten Level.");
            ChatAndTextUtil.sendNormalMessage(sender, (sender == player ? "Du hast " : (player.getName() + " hat "))
                    + data.getQuestPoints() + " Quest-Punkt" + (data.getQuestPoints() == 1 ? "" : "e") + ".");

            long streak = data.getDailyQuestStreak();
            ChatAndTextUtil.sendNormalMessage(sender,
                    (sender == player ? "Du hast " : (player.getName() + " hat ")) + "seit " + streak + " Tag"
                            + (streak == 1 ? "" : "en") + " immer min. eine DailyQuest abgeschlossen.");
        }
